...
Speedtest.prototype = {
  ...
  _checkServerDefinition: function(server) {
    try {
      ...
      if (server.server.indexOf("http://") != 0 && server.server.indexOf("https://") != 0 && server.server.indexOf("//") != 0) {
        if (server.server.charAt(0) == "/") {
          server.server = location.protocol + "//" + location.host + server.server;
        } else {
          let path = location.pathname;
          let dir = path.substring(0, path.lastIndexOf('/'));
          server.server = location.protocol + "//" + location.host + dir + "/" + server.server;
        }
      }
  ...
  selectServer: function(result) {
    ...
    const select = function(serverList, selected) {
      ...
      const checkServer = function(server, done) {
        let i = 0;
        server.pingT = -1;
        if (location.protocol === "https:" && server.server.indexOf("https:") == -1) done();
        else {
